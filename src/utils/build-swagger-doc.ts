...
const buildDefaultResponses = (params: SwaggerParams): object => {
...
const buildCustomResponses = (params: SwaggerParams): object => {
  const contentType = params.contentType ?? 'application/json'

  return params.errorMapping
    ? Object.entries(params.errorMapping).reduce<Record<number, any>>((acc, [code, { description, schema }]) => {
      acc[+code] = {
        description,
        content: {
          [contentType]: {
